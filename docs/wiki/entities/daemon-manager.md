...
- **开机自启**（`boot.ts`）：`installBootTask` / `uninstallBootTask`，经 `scripts/install-boot.ps1` / `uninstall-boot.ps1` 注册/移除**每用户登录**计划任务（免管理员、隐藏窗口）。有 `start-service.ps1` 时用它（git 安装路径稳定），否则回退 `cmd /c npx -y pi-weixin-bridge daemon start`。
